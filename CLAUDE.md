...
| `MATIERE[biome]` / `verifierTraces()` (`render/props.js`) | CE QUI EST ARRIVE AU SOL, par quartier. La tuile de `material.js` donne le MATERIAU, une fois par lieu ; ceci donne la MARQUE — six primitives partagees (rouler, souiller, empoussierer, cendrer, rayer, ruisseler), une grammaire et non un catalogue. Suit le QUARTIER donc l architecture ; `null` permis, c est du sol nu et le contraste en a besoin. La Nebuleuse n a ni poussiere ni roulage : sans gravite, ce qui marque une coque est ce qui l a HEURTEE. Sonde au CENTRE de la cellule, sous la grille de 20 m et sous les props. `verifierTraces()` refuse moins de deux matieres par lieu, et une primitive que plus personne ne tire |
