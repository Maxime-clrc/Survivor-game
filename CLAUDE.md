...
| `drawBrulure(...)` / `spawnBraise(...)` (`render/fx.js`) | CE QU'UN CORPS QUI BRULE DIT. Meme primitive que l'ombre (quad `fx_glow` teinte, lot NORMAL) et meme **passe separee**, pour la meme raison. La couleur vient de `STATUSES[STATUS_BURN]` — le HUD et la horde disent l'etat d'une seule voix. Les braises ont un budget **par IMAGE**, jamais par ennemi : la brulure se PROPAGE. **Aucune garde `gfx`** — une brulure est de l'INFORMATION, et `gfx` regle la matiere sans decider de ce qui se lit |
