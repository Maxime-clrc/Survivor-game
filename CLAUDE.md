...
npm run verif             # LA SUITE : 62 verificateurs, < 3 s
...
| `poidsAt(b, x, y, poids, durete)` / `pointMel()` / `loiTiree()` | LA REGION D UN POINT, ET C EST UN VECTEUR. `loiAt` rend l entier de la CELLULE — maille de 1600 x 900, soit une vue — donc une frontiere axiale, droite, longue d un ecran ; tout ce qui le lisait basculait au MEME pixel. `poidsAt` rend un poids par loi : au centre d une region un seul vaut 1, l identite reste ENTIERE. Le voisinage est le 2x2 des centres de cellule, donc UNE JONCTION DE TROIS REGIONS N EST PAS UN CAS PARTICULIER. Ce qui est une QUANTITE s interpole (teinte, densite, calibre, taux de baie), ce qui est un CATALOGUE se TIRE (`loiTiree`, par prop et jamais par cellule). `pointMel` est LE gauchissement — sol, semis, comblement de quartier et coupe de trame lisent le meme point deplace, sinon deux limites se dessinent. La largeur du fondu suit la PAIRE, par un exposant que `stage.js` construit : `biomes.js` ne depend de rien, il ne peut pas lire la palette |
| `drawFloor()` / `loisEnVue()` | LE SOL, PAR COUCHE DE REGION. Un motif est un `fillStyle`, il n a pas d opacite par pixel : on peint la region entiere hors ecran et on la ramene par un MASQUE (un texel pour 32 px — le champ est lisse, l interpolation du blit EST le fondu). La PREMIERE couche est opaque et les suivantes ont `w / S` : a leur poids nu le fond passe entre elles, `a1 + a2 = 1` ne disant rien de `(1-a1)(1-a2)`. Une seule region en vue ne paie RIEN, quatre est le maximum structurel. La marge de `loisEnVue` n est PAS la vue : une cellule qui ne la touche pas peut y peser par le gauchissement puis le noyau, et l oublier fait une bande de fond nu |
...
| `drawBrume()` / `voileBrume()` (`render/decor.js`, `stage.js`) | LA BRUME, ET ELLE EST UN CHAMP CENTRE SUR LE JOUEUR, jamais sur la vue. `voileBrume` RETIRE — la horde **et ce qu'elle a laissé au sol** ; `drawBrume` MONTRE — le même champ peint, **une seule passe**, posée entre `drawObstacles` et `drawZonesAnnonce`. Ce qui est dessiné AVANT est voilé (sol, semis, dangers, blocs, marquages, zones persistantes), ce qui est dessiné APRES ne l'est jamais (télégraphes, boss, alliés, marqueurs) : **c'est l'ordre de dessin qui porte la règle, pas un réglage**. Elle empruntait le vignettage, centré sur la VUE — les deux ne pouvaient pas coïncider, et l'effet n'avait aucune cause à l'écran |
| `drawZonesSol()` / `drawZonesAnnonce()` (`render/actors.js`) | LES DEUX MOITIES D'UNE ZONE, séparées parce que le voile de brume passe entre elles : ce qui est posé au sol est de la MATIERE, ce qui compte à rebours est une ANNONCE. `zonesRendues` est sans état et rejoué par les deux — un cache entre elles se désynchroniserait le jour où l'une est appelée seule |
...
| `AIR[theme][cle].matieres` / `verifierTraces()` (`render/props.js`) | CE QUI EST ARRIVE AU SOL, ET C EST LA CONSEQUENCE DE CE QUI EST ENCORE LA. La tuile de `material.js` donne le MATERIAU une fois par region ; ceci donne la MARQUE — vingt primitives partagees, une grammaire et non un catalogue. **DEUX MATIERES AU PLUS, ET LA SECONDE APPARTIENT A LA FAMILLE SIGNATURE** (`exclusivesDe`) : `matieres[0]` dit ce que la region fait au sol partout, `matieres[1]` ce que SON objet lui fait. Indexer par quartier de PROPS rendait la moitie des secondes injoignables — un quartier de props n a aucune raison d etre un quartier de bati. `null` permis, c est du sol nu et le contraste en a besoin. Sonde au CENTRE de la cellule, sous la grille de 20 m et sous les props ; `SONDE.kind` porte la famille, `SONDE.x/y` le point le plus proche. `tracesManquees()` mesure ce qui est VRAIMENT dessine — une primitive ecrite et injoignable ne leve rien |
| `ZONES[biome]` / `QUARTIER[biome]` / `verifierZones()` (`render/props.js`) | LES QUARTIERS D UN LIEU. `TABLE` dit ce que le lieu POSSEDE, `ZONES` comment il l ARRANGE, `QUARTIER` ce que l ARCHITECTURE attire autour d elle. Trois sources dans cet ordre : le bati a moins de `PORTEE_QUARTIER` impose son quartier, le hachage de la cellule DIVISEE comble le terrain libre, `FUITE` brouille la frontiere. Le repli sur le hachage EST le sens — stockage et circulation sont ce qui occupe l espace ENTRE les machines. Le rayon se MESURE : a 190 px l architecture tuait les quartiers qu elle ne dessert pas. `verifierZones()` croise les tables dans les deux sens — un prop qu aucune zone ne tire, ou une famille sans quartier, disparait en SILENCE |
...
| `ombre()` (`render/props.js`) | L OPACITE DE L OMBRE D UN PROP, et il n y en a qu une. `lumDir()` unifie la DIRECTION, ceci unifie la FORCE — dix-sept valeurs coexistaient, de 0,16 a 0,55, soit un facteur 3,4 entre deux objets poses cote a cote. Sept props n en ont AUCUNE et c est juste : une flaque, un marquage et un neon tombe n ont pas de volume. `npm run constantes-check` refuse un litteral a cette place |
| `hauteurDe(cle, kind)` (`render/blocs.js`) | LA HAUTEUR IMPLICITE D UN BLOC, en trois valeurs, et c est elle qui DECALE son ombre portee. Un decalage fixe faisait projeter la meme ombre a une bitte de 40 px et a une bande de trame de 3 680. La silhouette porte deja la hauteur : un mat est haut et fin, un mur bas est long et bas, une nappe est un creux. Trois valeurs et pas une echelle continue — au-dela, l oeil lit du bruit. `verifierBlocs` croise `SILHOUETTE` et `HAUTEUR` dans les deux sens : le repli est `H_MOYEN`, donc une silhouette oubliee prendrait l ombre d avant sans rien lever |
...
| `FOND` / `VITRAGE` / `VIE` (`material.js`, `decor.js`) | L ARRIERE-PLAN D UN LIEU : ce qui se CUIT, ce qui SEPARE, ce qui BOUGE. Trois tables sur le champ `fond` des `BIOMES`, croisees par `verifierFonds()` et `verifierBaies()` — une seule recette en dur et un `fond !== "espace"` faisaient qu un deuxieme lieu n affichait RIEN. Une baie est toujours PLEINE (verre ou caillebotis) : un trou franc dans le plancher ment. Et la couche « pres » n a pas le meme sens partout — une circulation immobile est une contradiction |
| `verifierCharte()` (`shared/palette.js`) | DEUX LIEUX NE PEUVENT PAS AVOIR LA MEME COULEUR. Croise les `BIOME_SKIN` deux a deux sur `arena`, `bloc`, `emis`, en LAB — deux hex proches en octets peuvent etre loin a l oeil. Les seuils sont les MINIMA deja acceptes par les quatre lieux d origine (6/9/8), donc il ne peut pas rougir sur l existant. `dir` n est pas verifie : on ne voit jamais deux lieux sur le meme ecran |
| `LED[biome]` / `verifierLed()` (`render/blocs.js`) | CE QU'UN BLOC EMET — teinte, rayon, seuil, et surtout le TYPE, qui porte un COMPORTEMENT dans `dessinerLed` : la chenille dit qu'une ligne TOURNE, donc elle n'appartient qu'a l'Usine. Table par lieu, jamais une chaine de ternaires a defaut implicite — le cinquieme lieu en heritait en silence. `verifierLed()` croise avec `BIOMES` dans les deux sens ET refuse deux lieux sous le meme `type`. Jamais sur une couverture destructible : le contour tirete est du gameplay. `evacDe` reste une chaine, et c'est juste — elle ECRIT son appartenance (`biomeKey() !== "usine"`) |
