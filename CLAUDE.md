...
**Les variantes de script changent la FORME de la pression, jamais sa QUANTITÉ.** C'est `adaptEntry` appliqué à un second axe, et pour la même raison : la quantité vit dans `diff.spawn` et nulle part ailleurs. Le plan demandait des débits abaissés en calme et relevés en cauchemar *en plus* du résidu — deux boutons sur la même grandeur, et un jour on règle l'un en croyant régler l'autre. Depuis le lot X il ne reste qu'**un** axe : **quelle géométrie** (calme retombe sur `bords`/`front`, cauchemar durcit vers `pince`/`quatre-fronts`). L'axe perdu — **où sont les silences** — était le plus fort des deux, et il n'a **pas** de successeur dans cette table : le seul substitut à portée était un débit par variante, c'est-à-dire précisément le second bouton qu'on refuse. Ce qui sépare réellement les modes vit dans le roster, les traits et le résidu ; si la mesure dit que calme et normal se ressemblent trop, le levier est là.

**Les variantes sont DÉRIVÉES de la table de référence, jamais recopiées** (`derive()` dans `timeline.js`). Trois tables de trente beats écrites à la main auraient divergé au premier réglage de débit — exactement le défaut qu'on vient de décrire. On décrit ce qui change ; le reste suit.

**LES SIX ÉTAPES PORTENT UN NOM** (`SEGMENT_NAMES` / `segmentName()` dans `timeline.js`) : Installation, Emprise, Crise, Ressac, Étau, Apothéose. « Segment 3/6 » est une **coordonnée** — elle dit où l'on est dans une liste, jamais ce qui s'y passe — et le mot était un terme d'implémentation qui avait fini au HUD, sur l'écran de cartes et sur le bilan. Le numéro reste **affiché à côté** : le nom situe dans l'histoire, le numéro dans la durée. Les identifiants du code gardent `segment`, qui est bien le nom de la structure.

**LE MODE PORTE LA CLARTÉ, LE BIOME PORTE LA TEINTE — un axe chacun** (plan6). Les deux se disputaient la même dimension : `DECOR[i].arena` distinguait les modes par la couleur, et `BIOMES[i].tint` devait distinguer les lieux par la couleur aussi. Résultat mesuré avant correction : `BIOMES[].tint` et `.grid` étaient **déclarés depuis le lot V et lus par personne** — vérifié en un grep, aucun appelant — donc les trois lieux d'un même mode avaient **littéralement la même couleur** (dE 0,00) ; et `calme` se séparait de `normal` de **dE 0,36**, c'est-à-dire rien, alors que le commentaire annonçait « ardoise franchement froide ». Deux axes annoncés, un seul tenu, et mal.

`teinter(base, teinte, k)` dans `palette.js` est le point de passage : il importe la **chroma** du biome en gardant la **clarté** du mode. Un `melange()` ordinaire ne pouvait pas convenir — les teintes de biome sont presque noires, donc mélanger revenait à assombrir, ce qui remet les deux informations sur la même dimension. La normalisation se fait sur la luminance **Rec. 709** et non sur la moyenne des canaux : le vert pèse six fois le bleu à l'œil, et une moyenne rendrait la friche nettement plus claire que l'usine à réglage égal.

Le réglage a été **cherché par balayage, pas deviné** — force du teintage × chroma des biomes, sous deux contraintes simultanées. Retenu : `PART_BIOME = 0.75`, chroma des biomes relevée d'environ moitié. Écarts minimaux obtenus : **dE 3,64 entre modes** à biome fixe, **dE 3,42 entre biomes** à mode fixe, et **dE 4,94 entre le sol et sa grille fine** — cette dernière n'est pas un détail, une grille qui se confond avec le sol supprime la graduation en mètres, qui est sa seule raison d'exister.

**La matière du sol est une TUILE CUITE, pas un shader** (`render/material.js`). Période de 400 px, c'est-à-dire exactement `GRID_MAJOR` : un joint de dalle qui tomberait entre deux traits marqués ferait deux quadrillages concurrents. La tuile est **transparente** — elle se compose par-dessus la couleur de sol, qui reste seule à porter mode et biome. Elle est cuite une fois par `(biome, mode, graine, densité)` et gardée en cache, donc le rendu par image redevient **un `fillRect`** là où la grille coûtait plusieurs centaines de tracés. Elle suit la densité de pixels de l'écran, comme l'atlas et pour la même raison.

Un vrai passe WebGL plein écran avait été envisagé et écarté : elle imposait un **quatrième canvas** sous `#cvUnder` (une composition de plus par image), un second programme GL avec sa propre gestion de perte de contexte, et un chemin de repli 2D à écrire en double. Une tuile cuite donne la même matière, fonctionne à l'identique dans les deux chemins de rendu, et se vérifie. Ce qu'elle ne saura pas faire — une lueur qui respire, une distorsion de chaleur — reste le jour où on le voudra la seule raison d'ouvrir ce passe.

`usure` est ce que le mode fait à la matière (0 en calme, 0,45 en normal, 1 en cauchemar) : rouille dans les angles de plaque, fissures chaudes, flaques sombres. Elle ne change **ni la teinte ni la géométrie** — c'est la règle ci-dessous appliquée au sol.

**Un OBSTACLE a une silhouette, et elle appartient à son biome.** Les trois biomes dessinaient le même `fillRect` gris : la géométrie changeait, la matière non, et c'est ce qui faisait lire les trois cartes comme la même carte à piliers déplacés. On applique donc au décor le **test de la silhouette** déjà exigé du bestiaire — pilier d'acier chanfreiné, cuve à pans coupés, bloc de béton ébréché, l'ébréchure déterministe par position pour qu'un bloc ne change pas de forme d'une image à l'autre. Le volume est une **face supérieure décalée vers le centre de la vue** : du 2,5D sans aucune image stockée, qui s'ouvre quand on traverse la salle. Le décalage est **plafonné** (7 px) et la face du sol est tracée **à la place exacte de la boîte de collision** — c'est elle qui dit la vérité au joueur, la face haute n'est qu'un ornement. Rien ne change côté simulation.
...
**UN NIVEAU OUVRE SON ÉCRAN DE CARTES** (lot X), et le boss ne donne plus de carte gratuite. Les montées attendaient la mort du boss qui clôt le segment : un niveau ne donnait donc **rien** au moment où on le gagnait, et plusieurs se cumulaient en une file d'écrans identiques. Une seule exception subsiste, le **combat de boss** — on ne le coupe pas en deux : les niveaux gagnés pendant restent en file et sortent à sa mort. L'écran arrête la simulation, donc il ne s'ouvre jamais « pendant qu'on esquive ». La carte garantie par boss (un `pendingLevels++` dans `_killBoss`) est supprimée : son argument tenait tant que le boss était le seul endroit où un écran pouvait s'ouvrir, et elle rendait sa récompense indépendante de ce qu'on y avait fait. Le boss reste un point d'étape par la **qualité** de tirage (`BOSS_QUALITY`, redevenu un vrai bonus puisque les écrans de niveau ne l'ont pas) et par le marchand.
...
**LES ACCALMIES ONT DISPARU au lot X, avec la prémisse qui les portait.** L'argument écrit était « sans nettoyage de vague, dans un **écran fixe** et **sans caméra**, la population tend vers `MAX_ENEMIES` et y reste » — et les deux conditions sont fausses depuis le lot I : arène de 4800 × 2700 pour une vue de 1600 × 900, caméra qui suit le joueur. La respiration vient du **déplacement**. Les quatre beats concernés ont repris leur place sur la courbe (moyenne des voisins) ; ce que l'accalmie portait en plus est repris ailleurs — la fenêtre de récupération par l'écran de cartes (qui s'ouvre désormais à chaque niveau), le bonus au sol forcé par `CFG.POWERUP_MIN`/`MAX`, seul réglage de cadence des bonus. Mesuré 1800 s de horde, solo, normal : la population moyenne **baisse** (123 → 106) et le temps passé au plafond aussi (41 % → 25 %), parce que les cartes arrivent deux fois plus vite. Ce que ça coûte est écrit dans `timeline.js` : les variantes de script n'ont plus que la géométrie pour se distinguer.
...
**Mesuré au lot X, et c'est le défaut ouvert du plan : le plafond est le VRAI régulateur de la fin de manche.** Il est atteint de 3 % à 64 % du temps selon la difficulté et l'effectif, par séries de deux à quatre minutes — donc les débits des derniers beats des segments 5 et 6 ne veulent plus rien dire, c'est le plafond qui règle. Ce n'est **pas** un problème d'effectif (3,4 % en calme à quatre contre 63,5 % en cauchemar à deux) mais un rapport entre le débit et ce que l'équipe sait nettoyer. Le CPU n'est pas la contrainte — p99 à 1,8 ms sur un budget de 16,7, l'O(n²) redouté a neuf fois de marge — la **lisibilité** l'est. Trois issues (relever le plafond, abaisser les derniers beats, assumer) ; l'arbitrage appartient au porteur du projet et n'est pas tranché.

**UN ENNEMI NE SE MATÉRIALISE JAMAIS SOUS LES YEUX** (`_pushOffScreen`, point de passage unique de `_edgePoint`). `_spawnBox()` est écrêtée aux bords de la salle — « au-delà il n'y a plus de terrain » — et c'est ce qui cassait l'invariant écrit à côté de `SPAWN_MARGIN` : collé à un mur, le côté correspondant de la boîte tombe **à portée de vue**, et la caméra étant clampée à la salle elle aussi, le joueur regarde exactement l'endroit où ça sort. Mesuré avant correctif : **15 % des apparitions dans le champ en solo, 34 % à quatre**, et fortement asymétriques (1901 à l'est contre 346 à l'ouest) — ce qui se lit comme « ils sortent tous du même côté », le défaut rapporté. **Le côté appartient au script, la distance à la lisibilité** : on ne change jamais de bord (`front` et `pince` perdraient tout leur sens), on repousse le point le long de l'axe de son bord jusqu'à sortir de toutes les vues, quitte à sortir de la salle — rien n'y borne un ennemi, il entre en marchant. Le rectangle de vue est **reconstruit** comme le client le calcule (centré puis clampé) et non supposé centré : c'est près des murs que les deux diffèrent, donc précisément là où le problème se pose. `anneau` en est exempt — seule géométrie qui fait naître à l'intérieur, délibérément, avec sa propre distance de garde. Les nuées de pondeuse aussi : elles naissent près de leur mère, donc à l'écran, et c'est la mécanique.
...
**Le bilan titre sur l'ÉTAPE atteinte — par son NOM — ou sur la VICTOIRE, pas sur
le numéro de manche.** « Morts à la Crise » est une phrase qu'une table se redit ;
« morts au segment 3 » est une coordonnée qu'il faut traduire. Le numéro reste
entre parenthèses, il dit la distance parcourue.
`roundNumber` s'incrémentait correctement — ce n'était jamais un bug de
